document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/models/ngos.json') // Assuming JSON file is served from the server
        ...
        .then(data => {
            ngos = data; 
            displayWomenRelatedNGOs(ngos); // Display all NGOs with women-related descriptions initially
        })
        .catch(error => {
            console.error('Error fetching ngos:', error);
        });

    function displayWomenRelatedNGOs(ngos) {
        ngoList.innerHTML = '';
        const womenRelatedNGOs = ngos.filter(ngo => ngo.description.toLowerCase().includes('women'));
        womenRelatedNGOs.forEach(ngo => {
            const ngoCard = `
                <div class="ngo">
                    <h3>${ngo.name}</h3>
    ...
    function searchNGOs() {
        ...
        if (searchQuery === '') {
            displayWomenRelatedNGOs(ngos); // Display all Women-related NGOs if search bar is empty
            return;
        }
        ...
        displayWomenRelatedNGOs(filteredNGOs);
    }
