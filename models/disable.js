document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/models/ngos.json') // Assuming JSON file is served from the server
        ...
        .then(data => {
            ngos = data; 
            displayDifferentlyAbledNGOs(ngos); // Display all NGOs with differently-abled-related descriptions initially
        })
        .catch(error => {
            console.error('Error fetching ngos:', error);
        });

    function displayDifferentlyAbledNGOs(ngos) {
        ngoList.innerHTML = '';
        const differentlyAbledNGOs = ngos.filter(ngo => ngo.description.toLowerCase().includes('abled'));
    ...
    function searchNGOs() {
        ...
        if (searchQuery === '') {
            displayDifferentlyAbledNGOs(ngos); // Display all Differently-Abled-related NGOs if search bar is empty
            return;
        }
        ...
        displayDifferentlyAbledNGOs(filteredNGOs);
    }
