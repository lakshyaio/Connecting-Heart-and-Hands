document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/models/ngos.json') // Assuming JSON file is served from the server
        ...
        .then(data => {
            ngos = data; 
            displaySocialRelatedNGOs(ngos); // Display all NGOs with social-related descriptions initially
        })
        .catch(error => {
            console.error('Error fetching ngos:', error);
        });

    function displaySocialRelatedNGOs(ngos) {
        ngoList.innerHTML = '';
        const socialRelatedNGOs = ngos.filter(ngo => ngo.description.toLowerCase().includes('social'));
        socialRelatedNGOs.forEach(ngo => {
            const ngoCard = `
                <div class="ngo">
                    <h3>${ngo.name}</h3>
    ...
    function searchNGOs() {
        ...
        if (searchQuery === '') {
            displaySocialRelatedNGOs(ngos); // Display all Social-related NGOs if search bar is empty
            return;
        }
        ...
        displaySocialRelatedNGOs(filteredNGOs);
    }
